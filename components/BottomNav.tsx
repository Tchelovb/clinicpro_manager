...
import {
  ...
  LogOut, Search, ChevronRight, User, Brain, Sparkles
} from 'lucide-react';
...
const BottomNav: React.FC = () => {
  ...
  const secondaryNavItems = [
    ...
    { to: '/dashboard/intelligence-gateway', icon: Brain, label: 'BOS Intelligence', desc: 'Portal de Inteligência Executiva', highlight: true },
    { to: '/dashboard/chatbos', icon: Sparkles, label: 'ChatBOS', desc: 'Consultoria em IA', highlight: true },
    // OPERATIONAL SECTION
            ...
            <div className="grid grid-cols-1 gap-2">
              {secondaryNavItems.map((item) => (
                <button
                  ...
                  className={`flex items-center gap-4 p-4 rounded-xl active:scale-[0.98] transition-all shadow-sm
                            ${item.highlight
                ...
                >
                  <div className={`p-3 rounded-xl shrink-0 ${item.highlight
                    ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white'
                    : location.pathname === item.to
                      ? 'bg-blue-600 text-white'
                      : 'bg-gray-100 text-gray-600'
                    }`}>
                    <item.icon size={22} className={item.highlight ? 'animate-pulse' : ''} />
                  </div>
                  <div className="flex-1 text-left">
                    <p className={`text-base font-bold ${item.highlight
                      ? 'text-purple-700'
                      : location.pathname === item.to
                        ? 'text-blue-700'
                        : 'text-gray-800'
