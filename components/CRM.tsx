...
import { 
  ...
  LayoutGrid, List, FileText, AlertCircle, RefreshCcw, ArrowRight, Briefcase, TrendingUp
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';

const CRM: React.FC = () => {
  const { leads, patients, sendToNegotiation } = useData();
  const navigate = useNavigate();
  const [activeView, setActiveView] = useState<'kanban' | 'list' | 'budgets'>('kanban');
  
  const conversionRate = (leads.filter(l => l.status === LeadStatus.WON).length / (leads.length || 1)) * 100;
  const totalValue = leads.reduce((acc, l) => acc + (l.value || 0), 0);
  
  const unapprovedBudgets = patients.flatMap(p => 
      (p.budgets || [])
        .filter(b => b.status === 'Em Análise' || b.status === 'Enviado' || b.status === 'Em Negociação') // Include 'Em Negociação' for visibility
        .map(b => ({ ...b, patientName: p.name, patientPhone: p.phone, patientId: p.id }))
  );

  const handleCreateOpportunity = (patientId: string, budgetId: string) => {
      const leadId = sendToNegotiation(patientId, budgetId);
      if (leadId) {
          navigate(`/crm/${leadId}`);
      }
  };
