...
import { Lead, LeadStatus, Interaction, InteractionType } from '../types';
import { ArrowLeft, Save, User, Phone, Mail, Globe, Clock, MessageCircle, AlertCircle, Send, Calendar, ChevronDown, CheckCircle, XCircle, HelpCircle, DollarSign, TrendingUp } from 'lucide-react';
...
export const CRMForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<Lead>>({ source: 'Indicação' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.name && formData.phone) {
      const lead: Lead = {
        ...
        interest: 'Médio', // Valor padrão
        createdAt: new Date().toISOString(),
        lastInteraction: new Date().toISOString(),
        history: [{
          id: Math.random().toString(36).substr(2, 5),
          type: 'System',
          content: 'Oportunidade criada manualmente.',
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 p-8 rounded-xl shadow-card border border-gray-200 dark:border-slate-700 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select className={inputClass} value={formData.source} onChange={e => setFormData({...formData, source: e.target.value as any})}>
        ...
        <div className="pt-6 flex justify-end gap-3 border-t border-gray-100 dark:border-slate-700">
            ...
            <button type="submit" className="px-6 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 flex items-center gap-2 font-medium shadow-sm transition-transform active:scale-95"><Save size={18}/> Salvar Contato</button>
        </div>
      </form>
    </div>
  );
...
export const CRMDetail: React.FC = () => {
  ...
  const handleStatusChange = (newStatus: LeadStatus) => {
    ...
    logHistory(`Etapa alterada de "${lead.status}" para "${newStatus}"`);
  };

  const handleInterestChange = (newInterest: 'Alto' | 'Médio' | 'Baixo') => {
      if (newInterest === lead.interest) return;
      updateLead(lead.id, { interest: newInterest });
      logHistory(`Interesse atualizado para: ${newInterest}`);
  };
  ...
  const handleValueBlur = () => {
      ...
      if (numValue !== lead.value) {
          ...
          logHistory(`Valor potencial atualizado para R$ ${numValue.toLocaleString('pt-BR')}`);
      }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 shrink-0">
        <div className="flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">{lead.source} • Criado em {new Date(lead.createdAt).toLocaleDateString()}</p>
            </div>
        </div>
      ...
      <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl shadow-card border border-gray-200 dark:border-slate-700 overflow-hidden flex flex-col transition-colors">
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {lead.history.map((interaction, idx) => (
                  <div key={interaction.id} className="flex gap-4 relative animate-in fade-in slide-in-from-bottom-2">
