...
const Financial: React.FC = () => {
            ...
            <div className="space-y-6 md:space-y-8 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Entradas Hoje</p>
                        <h3 className="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">R$ {todayIncome.toLocaleString('pt-BR')}</h3>
                    </div>
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Saídas Hoje</p>
                        <h3 className="text-2xl font-bold text-red-600 dark:text-red-400 mt-1">R$ {todayExpense.toLocaleString('pt-BR')}</h3>
                    </div>
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Resultado Operacional</p>
                    ...
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <h3 className="text-2xl font-bold text-slate-800 dark:text-white mt-1">R$ 450,00</h3>
                    </div>
                </div>
    ...
    const CaixaView = () => {
        ...
        return (
            ...
            ${currentRegister ? 'bg-green-50 border-green-200 dark:bg-green-900/10 dark:border-green-800' : 'bg-slate-50 border-slate-200 dark:bg-slate-800/50 dark:border-slate-700'}`}>
                    <div className="flex items-center gap-4">
                        <div className={`p-3 md:p-4 rounded-full ${currentRegister ? 'bg-green-100 text-green-600 dark:bg-green-900/50 dark:text-green-400' : 'bg-slate-200 text-slate-500 dark:bg-slate-700 dark:text-slate-400'}`}>
                            {currentRegister ? <Unlock size={24} className="md:w-8 md:h-8" /> : <Lock size={24} className="md:w-8 md:h-8" />}
                        </div>
                        <div>
                            <h2 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white">{currentRegister ? 'Caixa Aberto' : 'Caixa Fechado'}</h2>
                    ...
                    {!currentRegister ? (
                        <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto mt-2 md:mt-0">
                            <input
                                ...
                                className="border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-3 md:py-2 w-full md:w-40 bg-white dark:bg-slate-700 dark:text-white dark:placeholder-slate-400"
                                value={openAmount}
                        ...
                        <div className="w-full md:w-auto bg-white dark:bg-slate-700/50 p-3 rounded-lg border border-green-100 dark:border-green-900/30 md:border-0 md:bg-transparent">
                            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold">Saldo Atual</p>
                            <p className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white">R$ {currentRegister.calculatedBalance.toLocaleString('pt-BR')}</p>
                        </div>
                    )}
                </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
                                <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center">
                                    <h3 className="font-bold text-slate-800 dark:text-white">Movimentações</h3>
                                    <span className="text-[10px] text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-700 border dark:border-slate-600 px-2 py-1 rounded">ID: {currentRegister.id}</span>
                                </div>
                                <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-[400px] overflow-y-auto">
                                    {currentRegister.transactions.length > 0 ? (
                                        currentRegister.transactions.map(t => (
                                            <div key={t.id} className="p-4 flex justify-between items-center hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                <div className="flex items-center gap-3">
                                                    <div className={`p-2 rounded-full shrink-0 ${t.type === 'income' ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400'}`}>
                                                        {t.type === 'income' ? <ArrowUpCircle size={18} /> : <ArrowDownCircle size={18} />}
                                                    </div>
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-xs text-slate-500 dark:text-slate-400">{t.paymentMethod} • {t.category}</p>
                                                    </div>
                                                </div>
                                    ...
                                    ) : <div className="p-8 text-center text-slate-400">Nenhuma movimentação registrada hoje.</div>}
                                </div>
                            </div>

                            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 h-fit">
                                <h3 className="font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2"><Lock size={18} /> Fechamento</h3>
        ...
        return (
            <div className="space-y-4 md:space-y-6 animate-in fade-in">
                <div className="flex justify-between items-center">
                    <h3 className="font-bold text-slate-700 dark:text-white text-lg">Contas a Pagar</h3>
                ...
                <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block">Vencimento (De)</label>
                        <input
                            type="date"
                            className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 bg-white dark:bg-slate-800 text-sm dark:text-white"
                            value={payableFilters.startDate}
                            onChange={e => setPayableFilters(prev => ({ ...prev, startDate: e.target.value }))}
                        />
                    </div>
                    <div>
                        <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block">Vencimento (Até)</label>
                        <input
                            type="date"
                            className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 bg-white dark:bg-slate-800 text-sm dark:text-white"
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                className="w-full pl-9 pr-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm dark:text-white dark:placeholder-slate-500"
                    ...
                    <div>
                        <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block">Fornecedor</label>
                        <div className="relative">
                            <Search className="absolute left-3 top-2.5 text-slate-400" size={16} />
                            <input
                                type="text"
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                    ...
                    <div className="hidden md:block">
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-b dark:border-slate-800">
                                <tr>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {expenses.filter(expense => {
                                    ...
                                    <tr key={expense.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        <td className="px-6 py-4 font-mono text-slate-600 dark:text-slate-400">{expense.dueDate}</td>
                                        <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">{expense.description}</td>
                                        <td className="px-6 py-4"><span className="bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-xs text-slate-700 dark:text-slate-300">{expense.category}</span></td>
                                        <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{expense.provider}</td>
                                    ...
                                    ${expense.status === 'Pago' ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800' :
                                        ...
                                        <td className="px-6 py-4 text-right font-bold text-slate-900 dark:text-white">R$ {expense.amount.toLocaleString('pt-BR')}</td>
                            ...
                            <div key={expense.id} className="p-4 flex flex-col gap-3 relative border-l-4 border-transparent hover:border-red-500 transition-all">
                                <div className="flex justify-between items-start">
                                    <div className="flex-1 min-w-0 pr-2">
                                        <p className="font-bold text-slate-900 dark:text-white truncate">{expense.description}</p>
                                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{expense.provider} • {expense.category}</p>
                                    </div>
                                    <div className="text-right shrink-0">
                                        <p className="font-bold text-slate-900 dark:text-white">R$ {expense.amount.toLocaleString('pt-BR')}</p>
                                        <p className="text-xs text-slate-500 font-mono mt-0.5">{expense.dueDate}</p>
                                    </div>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border 
                    ...
                    {expenses.length === 0 && <div className="p-8 text-center text-slate-400">Nenhuma despesa registrada.</div>}
                </div>
            </div >
        );
    };
    ...
    const ReceivablesView = () => {
        ...
        return (
            <div className="space-y-4 md:space-y-6 animate-in fade-in">
                <h3 className="font-bold text-slate-700 dark:text-gray-200 text-lg">Contas a Receber</h3>
                ...
                <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block">Paciente</label>
                        <div className="relative">
                            <Search className="absolute left-3 top-2.5 text-slate-400" size={16} />
                            <input
                                type="text"
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {allReceivables.filter(item => {
                                    ...
                                    <tr key={`${item.id}-${idx}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        <td className="px-6 py-4 font-mono text-slate-600 dark:text-slate-400">{item.dueDate}</td>
                                        ...
                                        <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{item.paymentMethod}</td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border 
                                    ${item.status === 'Pago' ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800' :
                                        ...
                                        <td className="px-6 py-4 text-right font-bold text-slate-900 dark:text-white">R$ {item.amount.toLocaleString('pt-BR')}</td>
                                        <td className="px-6 py-4 text-right">
                                            {item.status !== 'Pago' && (
                    ...
                    <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-800">
                        {allReceivables.filter(item => {
                            ...
                            <div key={`${item.id}-${idx}`} className="p-4 flex flex-col gap-3 relative border-l-4 border-transparent hover:border-green-500 transition-all">
                                <div className="flex justify-between items-start">
                                    <div className="flex-1 min-w-0 pr-2">
                                        <p className="font-bold text-slate-900 dark:text-white truncate">{item.patientName}</p>
                                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{item.description}</p>
                                    </div>
                                    <div className="text-right shrink-0">
                                        <p className="font-bold text-slate-900 dark:text-white">R$ {item.amount.toLocaleString('pt-BR')}</p>
                                        <p className="text-xs text-slate-500 font-mono mt-0.5">{item.dueDate}</p>
                                    </div>
                                </div>
                                <div className="flex justify-between items-center">
                                    ...
                                    ${item.status === 'Pago' ? 'bg-green-50 text-green-700 border-green-100 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800' :
                                        ...
                                        <span className="text-[10px] text-slate-400 uppercase">{item.paymentMethod}</span>
                                    </div>
    ...
    return (
        <div className="space-y-6 pb-24 md:pb-0 flex flex-col h-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Gestão Financeira</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Controle de caixa, contas e resultados</p>
                </div>
                <button
                ...
                <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                    {[
                        ...
                        ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-sm' : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                        >
