  ...
  >
    <div className="bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-gray-100 w-full">
      ...
      <p className="text-slate-500 mb-8">
        Quem será o responsável por esta negociação?
      </p>
      ...
      <div className="space-y-5 text-left">
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setConfig({ ...config, professional: e.target.value })
            }
      ...
      <button
        onClick={onNext}
        className="w-full mt-8 bg-slate-900 hover:bg-black text-white py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 transition-transform active:scale-[0.98]"
      >
        ...
        <div className="text-right">
          ...
          <p className="text-2xl font-bold text-indigo-600">
            {formatCurrency(
              cart.reduce(
                (acc: number, item: any) =>
                  acc + item.unit_value * item.quantity,
                0
              )
            )}
          </p>
...
const StepNegotiation = ({
  ...
  const subtotal = cart.reduce(
    (acc: number, item: any) => acc + item.unit_value * item.quantity,
    0
  );
  ...
  return (
    ...
    >
      ...
      <div className="w-full md:w-4/12 bg-gray-50 p-8 border-r border-gray-200 hidden md:block">
        <button
          onClick={onBack}
          className="mb-6 flex items-center text-gray-500 hover:text-black gap-2 text-sm font-bold uppercase tracking-wider"
        >
        ...
        <div className="space-y-4 max-h-[400px] overflow-y-auto">
          {cart.map((item: any) => (
            ...
            >
              <span>
                {item.quantity}x {item.name} {item.region && `(${item.region})`}
              </span>
      ...
      <div className="w-full md:w-8/12 p-8 md:p-12 flex flex-col justify-center overflow-y-auto pb-64 md:overflow-visible md:pb-12">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-8 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <select
                ...
                className="w-full p-3 bg-gray-50 rounded-lg font-bold outline-none focus:ring-2 focus:ring-indigo-500"
                disabled={
                  negotiation.method === "PIX" ||
                  negotiation.method === "DINHEIRO"
                }
              >
                {[1, 2, 3, 4, 5, 6, 10, 12, 18, 24].map((x) => (
          ...
          <div className="mt-6 pt-6 border-t border-gray-100 flex items-center justify-between">
            ...
            <div className="text-right">
              <p className="text-sm text-indigo-600 font-bold uppercase">
                Valor da Parcela
              </p>
              <p className="text-3xl font-bold text-indigo-700">
                {negotiation.installments}x {formatCurrency(installmentValue)}
              </p>
            </div>
          </div>
        </div>
...
const StepReview = ({
  ...
  const installmentsList = Array.from({ length: installmentCount }).map(
    (_, idx) => {
    ...
    >
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col md:flex-row">
            <div className="w-full md:w-5/12 p-6 bg-slate-50 border-r border-gray-200 space-y-4">
              <h3 className="font-bold text-slate-800 text-sm uppercase mb-4">
                Resumo Financeiro
              </h3>
              ...
              <div className="border-t pt-4 mt-2">
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="font-bold">
                    {formatCurrency(negotiation.entry)}
                  </span>
                </div>
                <div className="flex justify-between text-sm mb-2">
            ...
            <div className="w-full md:w-7/12 p-6">
              ...
              {negotiation.installments > 0 ? (
                ...
                <div className="text-center p-4 text-gray-400 bg-gray-50 rounded-xl">
                  Pagamento à Vista
                </div>
              )}
            </div>
      ...
      <div className="bg-white border-t p-4 md:px-8 md:py-6 shadow-lg z-10 flex justify-end gap-3">
        <button
          onClick={onBack}
          disabled={isSaving}
          className="px-6 py-3 rounded-xl border border-gray-300 font-bold text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50"
        >
          Editar
        </button>
        <button
...
export function BudgetStudioPage() {
  const { id: patientId, budgetId } = useParams<{
    id: string;
    budgetId?: string;
  }>();
  ...
  const handleSave = async () => {
    ...
    try {
      await saveBudget("PENDING", {
        final_value:
          items.reduce((acc, i) => acc + i.unit_value * i.quantity, 0) *
          (1 - negotiation.discount / 100),
        status: "PENDING",
      });
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="h-[100dvh] bg-gray-50 flex flex-col font-sans overflow-hidden">
      <header className="h-16 bg-white border-b flex items-center justify-between px-4 md:px-8 shrink-0">
        <div className="font-bold text-lg md:text-xl tracking-tight text-slate-900">
          ClinicPro <span className="text-indigo-600">Studio</span>
        </div>
